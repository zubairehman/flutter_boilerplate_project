# lib/core/widgets/
## Responsibility
Shared UI primitive widgets used across multiple feature screens: styled text field, rounded button, loading indicator, empty app bar, and responsive app icon.
...
## Data & Control Flow
- `TextFieldWidget` → receives `TextEditingController`, `FocusNode`, callbacks (`onChanged`, `onFieldSubmitted`), displays `errorText` from form stores.
- `RoundedButtonWidget` → receives `onPressed` callback, optional asset image, customizable shape/border.
