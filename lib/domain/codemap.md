...
4. Data flows back: repository → use case → caller.
5. DI bootstrap order: `DataLayerInjection` → `DomainLayerInjection` (registers use cases) → `PresentationLayerInjection`.

## Integration Points

- **Upward (consumers)**: `presentation/` layer imports use cases and entities.
- **Downward (providers)**: `data/` layer implements abstract repositories and registers concrete instances with GetIt.
