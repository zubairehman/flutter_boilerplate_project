...
## Responsibility
Post list feature screen. `PostListScreen` is a `StatefulWidget` that fetches and displays a `ListView` of posts from `PostStore`. Loaded in the body of `HomeScreen`. PostStore is resolved as a factory (new instance per widget creation).
...
- **Service Locator**: `PostStore` resolved via `getIt<PostStore>()` (factory registration).
...
4. Error: `PostStore.errorStore.errorMessage` rendered via `FlushbarHelper.createError()` in `addPostFrameCallback` with `mounted` check.
